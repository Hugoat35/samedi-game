...
export default function BombGame({ roomCode, roomState, myPlayerId, isHost, players }: BombGameProps) {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      // Si le temps est écoulé, c'est l'hôte qui déclare l'explosion (pour éviter les doublons réseau)
  ...
  const submit = async () => {
    ...
    if (!res.ok) {
      ...
      setDraft(""); // On vide l'input si c'est bon
    }
  };
  ...
  const shakeAnimation = isPanic 
    ? { x: [-5, 5, -5, 5, 0], y: [-2, 2, -2, 2, 0], transition: { repeat: Infinity, duration: 0.2 } }
  ...
  return (
    <div className="flex flex-col items-center flex-1 max-w-md mx-auto w-full gap-4">
      ...
      {/* CONSIGNE */}
      {constraint && (
        <div className="w-full text-center p-4 bg-rose-500 rounded-2xl shadow-lg border-b-4 border-rose-700 text-white">
          <p className="text-sm font-semibold opacity-90 mb-1">
            {myTurn ? "À TON TOUR ! TROUVE UN MOT QUI :" : `Tour de ${players.find(p => p.id === currentTurnId)?.name} :`}
          </p>
      ...
      <div className="w-full p-4 bg-white/90 rounded-3xl shadow-md border border-slate-100 mt-auto sm:mt-4">
        <form onSubmit={(e) => { e.preventDefault(); submit(); }} className="flex flex-col gap-2">
          ...
          />
          {err && <p className="text-rose-600 text-sm font-bold text-center animate-in slide-in-from-top-1">{err}</p>}
          <button
            type="submit"
            disabled={!myTurn || busy || !draft}
